...
void app_main(void)
{
    ...
    while (1) {
        if (!receiving) {
            ...
            receiving = true;
            ESP_LOGI(TAG, "RX armed, waiting for CC1101 interrupts");
            ...
            if (hex_len >= sizeof(frame_hex)) hex_len = sizeof(frame_hex) - 1;
            for (size_t i = 0; i < plen && (i * 2 + 1) < sizeof(frame_hex); i++) {
                snprintf(&frame_hex[i * 2], 3, "%02X", rx_result.rx_packet[i]);
            }

            ESP_LOGI(TAG, "Packet OK: L=%u len(dec)=%u len(enc)=%u RSSI=%.1f LQI=%u CRC=OK Manuf=%s ID=%s Addr=%s state=0x%02X raw_rssi=0x%02X raw_lqi=0x%02X marc=0x%02X pkt=0x%02X",
                     ...
                     rx_result.rssi_raw, rx_result.lqi_raw, rx_result.marc_state, rx_result.pkt_status);
            log_hex("RAW(enc): ", rx_result.rx_bytes, rx_result.encoded_len ? rx_result.encoded_len : 3, 0);
            log_hex("RAW(dec): ", rx_result.rx_packet, rx_result.packet_size, 0);
        }
